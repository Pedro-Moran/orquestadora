...
public class AlphaTestStepDef {
	...
	@When("Execute TRX {string} con user perfilado")
	public void executeAPXTransaction382(String trxName) {
		String[] trxNameSplit = trxName.split("-");
		scenario.given("release_headers").tags("release_headers");
		scenario.given("kusu_kusut38201zz_headers").tags("kusu_kusut38201zz_headers");
		scenario.http().post("/APX_Runtime_Online/PGRestService")
				...
				.header("country-code","$kusu_kusut38201zz_headers.countryCode")
				.header("service-id","$kusu_kusut38201zz_headers.serviceId")
				.header("user-code","$kusu_kusut38201zz_headers.userCode")
				.header("aap","$kusu_kusut38201zz_headers.aap")
				.header("header-name","$kusu_kusut38201zz_headers.headerName")
				.header("version-code","$kusu_kusut38201zz_headers.versionCode")
				.header("client-document","$kusu_kusut38201zz_headers.clientDocument")
				.header("manager-user","$kusu_kusut38201zz_headers.managerUser")
				.header("txclass",getTxClass("$kusu_kusut38201zz_headers.txClass"))
				.header("logical-transaction-code", trxNameSplit[0]).body().json()
				.fromFile(trxName.concat(".json")).send();
	...
	@When("TRX return {string} is NOT NULL field")
    public void iAssertNotNull(String response1) {
        scenario.assertThat(response1).notNull();
    }
}