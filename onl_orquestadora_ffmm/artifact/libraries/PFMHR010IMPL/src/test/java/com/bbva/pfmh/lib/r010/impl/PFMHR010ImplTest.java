...
public class PFMHR010ImplTest {
    ...
    private FMC7Response outputFMC7WithoutContract(){
        FMC7Response response = ouputFMC7();
        response.getFfmm7().get(0).setIdContr(null);
        return response;
    }
    ...
    public void testExecuteGetFFMMStatements_ReturnsDataWhenContractFoundAndVisible() {
        ...
        Assert.assertFalse(result.isEmpty());
        Assert.assertEquals(1, result.size());
        Assert.assertTrue(result.get(0).getData().get(0).getIsVisible());
    }

    @Test
    public void testExecuteGetFFMMStatements_SinKusur() {
        InputListInvestmentFundsDTO input = new InputListInvestmentFundsDTO();
        input.setCustomerId("00001171");
        input.setPageSize(10);
        input.setPaginationKey("");
        ...
        Assert.assertNotNull(result);
        Assert.assertFalse(result.isEmpty());
        Assert.assertFalse(result.get(0).getData().get(0).getIsVisible());
    }
