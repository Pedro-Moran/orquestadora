...
public class AlphaTestStepDef {
	...
	@When("Execute TRX {string} con user perfilado de la kusut408")
	public void executeAPXTransactiont408(String trxName) {
		String[] trxNameSplit = trxName.split("-");
		scenario.given("release_headers").tags("release_headers");
		scenario.given("kusu_kusut40801zz_headers").tags("kusu_kusut40801zz_headers");
		scenario.http().post("/APX_Runtime_Online/PGRestService")
				...
				.header("country-code","$kusu_kusut40801zz_headers.countryCode")
				.header("service-id","$kusu_kusut40801zz_headers.serviceId")
				.header("user-code","$kusu_kusut40801zz_headers.userCode")
				.header("aap","$kusu_kusut40801zz_headers.aap")
				.header("header-name","$kusu_kusut40801zz_headers.headerName")
				.header("version-code","$kusu_kusut40801zz_headers.versionCode")
				.header("client-document","$kusu_kusut40801zz_headers.clientDocument")
				.header("manager-user","$kusu_kusut40801zz_headers.managerUser")
				.header("txclass",getTxClass("$kusu_kusut40801zz_headers.txClass"))
				.header("logical-transaction-code", trxNameSplit[0]).body().json()
				.fromFile(trxName.concat(".json")).send("respuestaTrx1");
	}
